...
def find_best_pretrained_model(test_env=None, pretrained_dir="./pretrained_weights"):
    """Find the best pretrained model in the pretrained weights directory."""
    
    if not os.path.exists(pretrained_dir):
        return None
    
    models = [f for f in os.listdir(pretrained_dir) if f.endswith('.pkl')]
    
    if not models:
        return None
    
    # If test_env is provided, look for environment-specific models first
    if test_env is not None:
        # Look for models that are NOT for the test environment
        # (we want models trained on other environments)
        available_envs = []
        for env_num in [0, 1, 2, 3]:
            if env_num != test_env:
                env_model = f"env{env_num}.pkl"
                if env_model in models:
                    available_envs.append(os.path.join(pretrained_dir, env_model))
        
        if available_envs:
            # Return the first available model from other environments
            return available_envs[0]
    
    # Fallback: Priority order for model selection
    priorities = ['best_f1', 'best_acc', 'best', 'model', 'env']
    
    for priority in priorities:
        matching_models = [m for m in models if priority in m.lower()]
        if matching_models:
            return os.path.join(pretrained_dir, matching_models[0])
    
    # If no priority matches, return the first model
    return os.path.join(pretrained_dir, models[0])
import warnings
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pretrained_model', type=str, default=None,
                        help='Path to pretrained model checkpoint (.pkl file)')
    ...
    # Load pretrained weights if provided or available
    pretrained_model_path = args.pretrained_model
    
    # If no pretrained model specified, try to find one automatically
    if pretrained_model_path is None:
        test_env = args.test_envs[0] if args.test_envs else None
        pretrained_model_path = find_best_pretrained_model(test_env=test_env)
        if pretrained_model_path:
            print(f"Auto-detected pretrained model: {pretrained_model_path}")
            print(f"   Using model trained on environments other than test env {test_env}")
    ...
    if pretrained_model_path is not None:
        ...
        try:
            ...
            if 'model_dict' in checkpoint:
                ...
                if 'model_hparams' in checkpoint:
                    print(f"   Original hparams: {checkpoint['model_hparams']}")
            else:
        ...
        print("  No pretrained model found. Training from scratch with base CLIP weights.")
        print("   To use pretrained DR-specific weights, place them in ./pretrained_weights/")
        print("   Expected filenames: env0.pkl, env1.pkl, env2.pkl, env3.pkl")
