...
# from sklearn.metrics import plot_confusion_matrix  # Deprecated in newer sklearn
try:
...
def confusionMatrix(network, loader, weights, device, output_dir, env_name, algo_name,args,algorithm_class,dataset,hparams):
    ...
    for i in range(trials):
        ...
        # print(conf_mat, 'cf_matrix')  # Commented out to reduce terminal output
    conf_mat=(conf_mat_all[0]+conf_mat_all[1]+conf_mat_all[2])/(trials*1.0)
    conf_mat=conf_mat.astype('int')
    # print(conf_mat, 'cf_matrix_average')  # Commented out to reduce terminal output
...
def TsneFeatures(network, loader, weights, device, output_dir, env_name, algo_name):
    ...
    # print(labels)
    # print(labels.shape)  # Commented out to reduce terminal output
...
def make_weights_for_balanced_classes(dataset):
    ...
    classes = []
    for _, y in dataset:
        # print(y)  # Commented out to reduce terminal output
        y = int(y)
        counts[y] += 1
        classes.append(y)
...
def plot_confusion_matrix_custom(network, loader, weights, device, class_names=None, output_dir=None, step=None, env_name=None):
    """
    Plot confusion matrix for the current model predictions.
    
    Args:
        network: The trained model
        loader: Data loader for evaluation
        weights: Sample weights (can be None)
        device: Device (cuda/cpu)
        class_names: List of class names for labeling
        output_dir: Directory to save the plot
        step: Current training step
        env_name: Environment name for the plot title
    ...
    # Default class names for DR dataset
    ...
    cm = confusion_matrix(list_gt, list_pred, labels=[0, 1, 2, 3, 4])
    ...
    plt.title(f'Confusion Matrix - {env_name or "Unknown"} - Step {step or "Unknown"}')
    ...
    if output_dir is not None:
        ...
        filename = f'confusion_matrix_{env_name or "unknown"}_step_{step or "unknown"}.png'
