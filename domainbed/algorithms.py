...
from domainbed.lib.misc import (
    ...
    MovingAverage, l2_between_dicts, proj,Weighted_Focal_Loss, FocalLoss
)
import warnings
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, weights_for_balance):
        ...
        self.weights_for_balance = weights_for_balance
        # print('b'*100)
        self.weights_for_balance  = torch.tensor([self.weights_for_balance[0],self.weights_for_balance[1],self.weights_for_balance[2], self.weights_for_balance[3], self.weights_for_balance[4]])
        # print('c'*100)
        self.weights_for_balance = self.weights_for_balance.to(torch.float)
        # self.weights_org = self.weights_org.to(device)
        # print('d'*100)
        print('self.weights', self.weights_for_balance)
    def update(self, minibatches, unlabeled=None):
...
class ERM_focal(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams, weights_for_balance):
        super(ERM_focal, self).__init__(input_shape, num_classes, num_domains,
                                  hparams, weights_for_balance)
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
        ...
        self.weights_for_balance = self.weights_for_balance.to(torch.float)
        print('self.weights', self.weights_for_balance)
        self.lossfocal = Weighted_Focal_Loss(weights_for_balance = self.weights_for_balance, gamma = 2.0)

    def update(self, minibatches, unlabeled=None):
        all_x = torch.cat([x for x, y in minibatches])
        all_y = torch.cat([y for x, y in minibatches])
        #focal loss
        loss=self.lossfocal(self.predict(all_x), all_y)
...
class ERM_for_class_imbalance(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM_for_class_imbalance, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
...
class ERM_ViT_learning_probing(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """
...
# # Get encoded sentence as a PyTorch tensor
# encoded_sentence = outputs.pooler_output
...
class Clip_train_advanced(Clip_train):
    """
    Empirical Risk Minimization (ERM)
    """
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, weights_for_balance):
        super(Clip_train_advanced, self).__init__(input_shape, num_classes, num_domains,
                                  hparams, weights_for_balance)
...
class Biomedical_Clip_train(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, weights_for_balance):
        super(Biomedical_Clip_train, self).__init__(input_shape, num_classes, num_domains,
                                  hparams, weights_for_balance)
...
class Biomedical_Clip_train_advanced(Biomedical_Clip_train):
    """
    Empirical Risk Minimization (ERM)
    """
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, weights_for_balance):
        super(Biomedical_Clip_train_advanced, self).__init__(input_shape, num_classes, num_domains,
                                  hparams, weights_for_balance)
...
class label_smooth_loss(torch.nn.Module):
    def __init__(self, sd=0.2):
        ...
        # self.loss_weights=torch.tensor([torch.Tensor([0.9999962733607158, 3.726639284186568e-06, 1.9287426602089785e-22, 1.3863381272398307e-49, 1.383891369453615e-87]), torch.Tensor([3.726625396397968e-06, 0.9999925467492071, 3.726625396397968e-06, 1.928735472507598e-22, 1.3863329608769576e-49]), torch.Tensor([1.928735472507598e-22, 3.726625396397968e-06, 0.9999925467492071, 3.726625396397968e-06, 1.928735472507598e-22]), torch.Tensor([1.3863329608769576e-49, 1.928735472507598e-22, 3.726625396397968e-06, 0.9999925467492071, 3.726625396397968e-06]), torch.Tensor([1.3863329608769576e-49, 1.928735472507598e-22, 3.726625396397968e-06, 0.9999925467492071, 3.726625396397968e-06])])
        arr=np.array([
                [0.8805368899664596, 0.1191677094038969, 0.00029538721907325344, 1.3410558998661433e-08, 1.1151264946619767e-14],
                [0.10647886675301818, 0.786778319788613, 0.10647886675301818, 0.00026393472273301093, 1.1982617873959611e-08],
                [0.0002638650827373542, 0.10645077197359154, 0.7865707258873424, 0.10645077197359154, 0.0002638650827373542],
                [1.1982617873959608e-08, 0.0002639347227330109, 0.10647886675301817, 0.7867783197886128, 0.10647886675301817],
                [1.1151264946619768e-14, 1.3410558998661435e-08, 0.0002953872190732535, 0.11916770940389691, 0.8805368899664597]]
        )
        
        # print(arr)
        self.loss_weights=torch.tensor(arr)
        self.loss_weights = self.loss_weights.to('cuda')
    
    def forward(self, pred, target):
        pred = pred.log_softmax(dim=1)
        
        true_dist=self.loss_weights[target]
        
        return torch.sum(-true_dist * pred, dim=1).mean()


class Biomedical_Clip_train_label_smoothing(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, weights_for_balance):
        super(Biomedical_Clip_train_label_smoothing, self).__init__(input_shape, num_classes, num_domains,
                                  hparams, weights_for_balance)
        ...
        self.Class_names=["No DR","mild DR", "moderate DR", "severe DR", "proliferative DR"]
        self.Loss_fn=label_smooth_loss(sd=0.2)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        logits_per_image = logit_scale * image_features @ text_features.t()
        loss=self.Loss_fn(logits_per_image,all_y)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        self.cnt+=1
...
class Clip_zero(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(Clip_zero, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
...
class Biomedical_Clip_train_focal(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, weights_for_balance):
        super(Biomedical_Clip_train_focal, self).__init__(input_shape, num_classes, num_domains,
                                  hparams, weights_for_balance)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        loss = self.focal_loss(logits_per_image, all_y)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        self.cnt+=1
...
class Biomedical_Clip_train_prompt(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams, weights_for_balance):
        super(Biomedical_Clip_train_prompt, self).__init__(input_shape, num_classes, num_domains,
                                       hparams, weights_for_balance)
        ...
        tokenizer = transformersv2.AutoTokenizer.from_pretrained("dmis-lab/biobert-base-cased-v1.1")
        self.biobert_mlp_model = transformersv2.AutoModel.from_pretrained("dmis-lab/biobert-base-cased-v1.1")
        self.biobert_mlp_model = self.biobert_mlp_model.to(device)
        
        self.optimizer = torch.optim.AdamW(
            list(self.featurizer.visual.parameters()) + list(self.biobert_mlp_model.parameters()),
            lr=self.hparams["lr"],
            weight_decay=self.hparams['weight_decay']
        )
        self.Class_names=["No DR","mild DR", "moderate DR", "severe DR", "proliferative DR"]
        
        for param in self.biobert_mlp_model.parameters():
            param.requires_grad = False
        self.biobert_mlp_model.embeddings.prompt_tokens.requires_grad = True
        
        sentences = [tokenizer(f"a photo of a {c}", return_tensors="pt")['input_ids'] for c in self.Class_names]
        max_token_length = max([s.shape[1] for s in sentences])
        sentences = [tokenizer(f"a photo of a {c}", max_length=max_token_length, padding='max_length', truncation=True, return_tensors="pt")['input_ids'] for c in self.Class_names]
        self.sentences = torch.cat(sentences, 0).to(device)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        image_features = image_features / image_features.norm(dim=1, keepdim=True)
        text_features = self.biobert_mlp_model(self.sentences).pooler_output
...
class Clip_train_prompt_from_image(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """
    
    def __init__(self, input_shape, num_classes, num_domains, hparams, weights_for_balance):
        super(Clip_train_prompt_from_image, self).__init__(input_shape, num_classes, num_domains,
                                  hparams, weights_for_balance)
